...
class CorrelationRequest(BaseModel):
    ...
    period_days: int = Field(default=90, ge=7, le=365, description="Analysis period in days")
    timeframe: str = Field(default="daily", description="Data timeframe (daily, weekly)")
...
class CorrelationPair(BaseModel):
    ...
    p_value: Optional[float] = Field(default=None, description="Statistical significance")
    sample_size: int
...
class RollingCorrelationRequest(BaseModel):
    ...
    period_days: int = Field(default=180, ge=30, le=730, description="Total analysis period")
...
class BetaCalculationResponse(BaseModel):
    ...
    correlation: float
    interpretation: str
    volatility_ratio: float = Field(description="Asset volatility / Benchmark volatility")
...
class DiversificationScore(BaseModel):
    ...
    max_correlation: float
    min_correlation: float
    diversification_score: float = Field(ge=0, le=100, description="0=poor, 100=excellent")
